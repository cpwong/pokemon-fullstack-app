...
export default function PokemonInfo({ pokemon }) {
  ...
  const mutatationUpdatePokemon = useUpdatePokemon();
  ...
  const updateQuantity = (id, newQuantity) => {
    ...
    mutatationUpdatePokemon.mutate({ id, quantity: newQuantity });
  };
  ...
  return (
    <div className='relative border rounded-lg text-center cursor-pointer transition hover:border-x-cyan-200 hover:border-y-purple-200 hover:shadow-lg'>
      ...
      {showDetail === pokemon.id ? (
        <div className='absolute top-0 left-0 rounded-lg w-full h-full backdrop-blur-sm flex flex-col justify-center items-center'>
          <button
          ...
          <button
            ...
            disabled={mutatationUpdatePokemon.isLoading}
          ...
          <div className='w-full border-t'>
            <button
              className='flex items-center justify-center rounded-lg bg-yellow-200 text-black font-bold px-4 py-2 transition hover:bg-yellow-300 mx-auto mt-3'
              disabled={mutatationUpdatePokemon.isLoading}
